      ...
      return (
        <View style={[styles.contain, style]}>
          {data.map((item, index) => {
            if (index == 0) {
              return (
                <View style={[styles.contentCenter, contentLeft]} key={index}>
                  {renderValue(type, '7')}
                  {renderTitle(type, 'Friends')}
                </View>
              );
            } else if (index == data.length - 1) {
              return (
                <View style={[styles.contentCenter, contentRight]} key={index}>
                  {renderValue(type, '+125.47')}
                  {renderTitle(type, 'Balance')}
                </View>
              );
            } else {
              return (
                <View style={[styles.contentCenter, contentCenter]} key={index}>
                  {renderValue(type, '128')}
                  {renderTitle(type, 'Credit Score')}
                </View>
              );
            }
          })}
